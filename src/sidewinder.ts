...
export function* sidewinderGenerator(grid: Grid) {
    for (const row of grid.eachRow()) {
        const run = [] as Cell[];
        for (const cell of row) {
            run.push(cell);
    ...
    function explain(explanation: string, run: Cell[], carveFrom?: Cell, carveTo?: Cell) {
        const decorators = [] as CellDecorator[];
        if (carveTo) {
            decorators.push({
                cell: carveTo,
                render: (rect, context) => {
